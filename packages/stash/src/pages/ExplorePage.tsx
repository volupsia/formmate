...
const TabContent: React.FC<TabContentProps> = ({ queryName, sheetRef, onBookmark }) => {
  const url = `${apiBaseUrl}/api/queries/${queryName}`;
  ...
  const handleSpeak = async (item: TopListItem, index: number, autoPlay: boolean = true) => {
    ...
    try {
      const response = await fetch(
        `${apiBaseUrl}/api/queries/contentTag?entityName=${item.entityName}&recordId=${item.recordId}`
      );
          ...
          <div className="flex flex-col items-center gap-2 shrink-0">
            <button
              onClick={(e) => {
                ...
                e.stopPropagation();
                if (item.entityName) {
                  onBookmark({ entityName: item.entityName, recordId: item.recordId });
                }
              }}
