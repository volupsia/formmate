...
* entity_generator: Create or modify entities, content types, or relationships between entities.
* data_generator: Generate data for a specific entity.
* query_generator: Create or add one or more GraphQL queries or operations.
...
* If the input refers to generating example data, mock data, or items for an entity → data_generator.

* If the input asks for a GraphQL query without explicitly saying add/create → @query_generator.
* If the input includes creating, adding, or generating a query (e.g., "add query", "create query", "getAll*", "get*ById", or mentions GraphQL operations) → query_generator.
